...
`vjassc` is a C++20 phase-4 compiler prototype for lowering a supported subset of vJASS/Zinc to plain Warcraft III JASS.
...
Phase 4 adds `function interface` parsing/lowering, function-object `.execute/.evaluate`, function-object `.name`, static-method function references, callback trigger wrappers, and non-capturing Zinc anonymous function lowering for the supported fixture subset. It is still not a complete JassHelper replacement: large lambda-heavy full codegen, interface dispatch, delegate, operator overloads, stub/super, capturing closures, byte-for-byte output matching, and pjass validation remain future work.
...
build/vjassc samples/input.j --scan-only --allow-unsupported --emit-stats build/input.phase4.stats.json
build/vjassc tests/fixtures/phase4_function_interface_execute.in.j -o build/phase4_function_interface_execute.out.j
```

## Phase Boundary

Phase 4 supports small vJASS/Zinc function-interface, function-object, callback, and non-capturing-lambda cases in its declared subset. It intentionally refuses full code generation for large lambda-heavy inputs and for unsupported future syntax. `--allow-unsupported` is only for scan-only validation and statistics; it does not make partial code generation safe.
