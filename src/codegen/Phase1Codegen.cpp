...
CodegenResult Phase1Codegen::generate(const Program& program) {
    ...
    writer_.writeln("// Generated by vjassc phase2");
    writer_.writeln();
...
void Phase1Codegen::emitStructSupportFunctions(const StructInfo& info) {
    ...
    const MethodInfo* onDestroy = findMethod(info, "onDestroy");
    if (!info.isArrayStruct) {
        ...
        writer_.indent();
        if (onDestroy) {
            writer_.writeln("call " + onDestroy->generatedName + "(this)");
        }
...
void Phase1Codegen::collectStruct(const Decl& decl, const Decl* container) {
    ...
    for (const auto& method : decl.methods) {
        ...
        methodInfo.generatedName = method.isOnDestroy ? ("sc__" + info.generatedName + "_onDestroy") : (info.prefix + "_" + method.name);
