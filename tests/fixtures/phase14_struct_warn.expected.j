...
function s__Bag__allocate takes nothing returns integer
    ...
    if (this>818) then
        call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Unable to allocate id for an object of type: Bag")
        return 0
    endif
...
function s__Bag_destroy takes integer this returns nothing
    if this==null then
        call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Attempt to destroy a null struct of type: Bag")
        return
    elseif (si__Bag_V[this]!=-1) then
        call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Double free of type: Bag")
        return
    endif
